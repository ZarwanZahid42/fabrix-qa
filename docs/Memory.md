...
| Active stage | Phase 2 detector prototype training complete: supplied v2 adopted; scoped Phase 3 file-video inference implemented; full phase exit gates and Phase 1 product decisions remain open |
| Product implementation | Offline preprocessing, trained detector image inference and annotated file-video processing; website product features and live camera integration not started |
| Next task | Review prototype outputs; obtain Colab run metadata to reconcile provenance, gather independent real edge examples, and define ROI/live-frame integration contracts before backend integration |
| Team/product constraint | Two-person student team; website only; one FastAPI backend |

The repository contains a minimal buildable Next.js shell, a FastAPI health bootstrap with one real smoke test, local Docker Compose topology, pinned requirements, an offline dataset preprocessing pipeline, prototype YOLO image/file-video inference with tests, and six living documentation files. It does **not** contain authentication, database models/migrations, live camera capture/ROI, CNN/autoencoder fusion, scoring, grading, alerts, reports or dashboard product features.
...
- Baseline v2 is now supplied at `ai/models/best.pt` with `validation_metrics.json`, and is the adopted prototype detector. Exact report mAP50 is 0.7397368457536011 and mAP50-95 is 0.46414946906810284. Checkpoint loading and real image/video inference are verified locally. The Colab notebook remains the training entry point; local training runners and live ROI patch extraction remain placeholders. Historical v1 infinite loss is not diagnosed by successful v2 inference.
- `ai/inference/run_detection.py` provides shared checkpoint loading, exact nine-class-map checks, BGR frame inference, original-pixel boxes and image annotation. `edge/stream_handler.py` uses it for bounded every-frame file-video annotation, verified encoding, per-frame logs and model provenance. ROI selection, live capture/reconnect, tracking and backend integration remain placeholders.
- Five local raw datasets contain 96,885 training candidate source images (plus two excluded TILDA reference images). The AITEX-tiled real baseline has 156,371 outputs. A disclosed, verified 390-image synthetic train-only extension brings the active total to 156,761: 5,025 semantic detector images, 151,734 unchanged ZJU anomaly images and two classification-only positives. AITEX still has 530 outputs (528 detector tiles/copies plus two untiled classification-only strips). Raw/generated payloads, weights, inference outputs and recovery archives remain ignored by Git. The supplied v2 checkpoint/report are local; complete Colab run metadata/logs and v1 artifacts are not. Scoring logic and live camera processing do not exist.
...
v2 training is now complete for the prototype and its supplied checkpoint is the
default for image/file-video processing. Exact training-data lineage and historical
nonfinite loss still need run-artifact inspection, not speculative fixes. A real-only strong-label
...
- Verification scope at that date was explicit: `synthetic-verification.json` reported the new layer plus preservation checks; `verification.json` still represented the earlier complete decode of the 156,371-image real baseline. The entire unchanged ZJU population was not decoded again in that task. The 2026-09-12 full verification below supersedes that report. Config `real_manifest_sha256` hashes normalized JSONL records rather than platform-specific line-ending bytes.
...
## 2026-09-12 — Baseline v1 report and v2 preparation

- **User-reported, not independently verified:** validation mAP50 0.644, mAP50-95 0.440; epoch 28 reported as best, 43 total epochs run with patience 15 (distinguish best epoch from final stop). Class mAP50: stain 0.957, foreign_object 0.824, hole 0.778, crease 0.665, pattern_break 0.663, weave_error 0.568, edge_damage 0.055 with reported zero recall. Infinite box loss reported at epochs 20, 28, 31, 35, 38–43. No raw results log, actual Colab runtime versions or failing checkpoint/batch was supplied; no fabricated artifact hashes or confirmed loss cause are inferred from this summary.
- The user fixed the extraction and run-metadata ZIP variable references manually in Colab. The checked-in notebook still consistently used the older bare `DATASET_ZIP` name, including its definition. Standardized **all five occurrences** (definition, extraction, run metadata, instructions and error message) to the requested `DATASET_ZIP_PATH`, rather than assuming the repo definition already had the suffix. Added a whole-notebook regression check. Run names/metadata/date now identify a **v2 candidate**; warnings make clear that loss stability is unresolved. No AMP, optimizer, learning-rate or package changes were guessed.
- **Edge diagnosis:** visually compared five fixed synthetic v1 images with five unaugmented AITEX training tiles. Old notches are thin, repetitive sawteeth anchored strictly to the image border, unlike the short inward cuts present in real training labels. This is a plausible domain/position shortcut, not proof of the model's failure cause. A concrete generator flaw was identified: contrast and fiber colors were sampled from the top border before rotating the overlay onto another destination, so visibility could be based on unrelated fabric pixels.
- **Edge code revision:** `render_edge_v2` selects orientation before destination-region color sampling, generates coherent tapered notches or inward near-edge cuts, uses approximately 100 levels of luminance contrast, a narrow worn rim and short fibers, and derives labels from exact changed pixels. Recipe probabilities are 30% border notch / 70% inward cut. Keep 164 synthetic edge images (200 total edge train images); do not increase synthetic dominance without evidence. Crease/foreign_object rendering, existing backgrounds, source-parent/split assignments and real annotations remain unchanged. The images remain procedural proxies, not proven textile realism or measured recall gains.
- `refresh_synthetic_edge.py` provides explicit staged, recoverable v1-to-v2 replacement. It verifies legacy samples, stages new image/mask/box triples, confines all move targets, archives old payloads/manifests, checks all non-edge records/payload metadata and held-out content hashes, and updates synthetic provenance/config fingerprints. Family version remains stable for unrelated class seeds; edge rows explicitly carry recipe revision 2. A forced-failure regression confirmed rollback restores payload bytes and the original manifest. Preview evidence is under `_sanity_check/edge_real_train_review.jpg`, `edge_v1_review.jpg`, and `edge_v2_comparison.jpg`; no held-out appearance was used to tune the recipe.
- **Loss investigation: genuinely inconclusive.** `audit_yolo_labels.py` scans every semantic label, checking finite values, class IDs, normalized centers/sizes and box corners. Before regeneration, all **5,025 files / 4,679 boxes** pass. Eight known TILDA train boxes have positive subpixel dimensions (~0.8333 × 0.8340px minimum); no AITEX or synthetic boxes are degenerate. The audit initially included classification-only manifest entries outside its labels/ scope, causing two false missing-file warnings; fixed the auditor's subset selection, not dataset labels. No boxes were silently dropped/enlarged or source conversion logic changed.
- CPU float32 CIoU forward/backward on **all 4,342 train boxes**, strides 8/16/32, and three controlled finite prediction offsets remained finite (`_sanity_check/box_loss_probe.json`). This does not reproduce actual model activations, assignment, optimizer/AMP state or the Colab failing batch. Official PyTorch AMP troubleshooting supports float32 isolation as a diagnostic; it does not establish AMP as this run's cause. References and scoped conclusions are in the dataset README. Requested `run_metadata.json`, `pip-freeze.txt`, `args.yaml`, `results.csv`, and eventual checkpoint/failing-batch evidence for further diagnosis. No local GPU is available.
- **Model status:** v1 best.pt remains on the user's Drive, not in this repo or locally inspected. Preserve it as historical evidence; it should be superseded by v2 only once fixes and finite-loss retraining are confirmed. A valid dataset alone does not certify training stability. Real-only/strong-label ablations, independent-source evaluation and final-test protocol remain open.
- **Migration completed:** replaced exactly 164 synthetic edge image/mask/label triples (53 border notches, 111 inward cuts). Every other manifest record is identical; all 156,597 non-edge outputs retain matching size/mtime snapshots, and all held-out image/label/mask contents retain matching hashes. Old payloads and metadata remain recoverable under ignored `processed/_edge_v2_d5g8byof/previous/`. Current manifest SHA-256: `160705b5c2ba87051dbb2600f8db2d8f2863326575ed3847814195352d29705a`; source manifest unchanged. Actual post-migration synthetic contact images were inspected and labels align with the inserted regions.
- **Post-change checks:** all 5,025 semantic label files / 4,679 boxes pass the exhaustive geometry audit; all nine bounded CPU CIoU probe combinations remain finite. Ultralytics accepts every semantic image/box with a rebuilt train cache, and the existing 16-image-per-split binary loader smoke check passes. Notebook preflight passes against the revised manifest; no Colab training was executed locally.
- **Code verification:** 23 preprocessing and six notebook tests pass through the AI environment's standard-library `unittest` runner. An attempted pytest invocation found pytest absent from that environment; no dependency was added because these tests use unittest. Ruff passes; Black checks all 15 Python files plus an explicit formatting check of all seven notebook code cells (the Black CLI skips notebooks without its optional Jupyter extras). AI `pip check` passes. Backend, frontend and CI configuration were not changed or rerun in this AI-only task.
- **Full end-to-end verification passed:** `build_dataset.py --verify` independently decoded all **156,761 outputs**, including all unchanged ZJU images, and verified labels/masks, file inventory, source accounting, AITEX source-mask coverage and split isolation. Current `verification.json` now covers the combined real-plus-synthetic dataset and matches manifest `160705b5c2ba87051dbb2600f8db2d8f2863326575ed3847814195352d29705a`. Zero erased masks or cross-split parent/source-pixel overlap; only the eight documented positive TILDA subpixel boxes remain. Counts are unchanged: semantic train/val/test **4,607/210/208**, anomaly **132,766/9,484/9,484**, plus two classification-only train strips.
- **Handoff:** data and notebook are ready for a diagnostic v2 Colab run using a newly uploaded ZIP and fresh extraction, not certified stable training. No ZIP, GPU run or replacement checkpoint was produced here. Inspect v1 artifacts and isolate the nonfinite loss before accepting v2 as the replacement model. Remained on `features/zarwan`; no commit, push, protected-branch or ruleset change. Generated data, review images and recovery archives remain Git-ignored.

## 2026-09-13 — Supplied baseline v2 adopted and prototype inference verified

- Read Memory/Rules/Phases and relevant architecture/product/design guidance. Stayed on dirty `features/zarwan`, preserving the prior dataset/notebook changes and user-supplied report/checkpoint. `run_detection.py` did not exist (not merely a placeholder); created it. Existing `defect_detector.py` remains the future fused-pipeline placeholder, not a competing pretrained detector.
- **Training complete for the prototype:** adopted Zarwan's supplied `ai/models/best.pt` as the model going forward. SHA-256 `d13aa10c5d724a8e5b431855ff241b4bd3dbf4ce17fb1f26cdb95e1939b8e376`; 6,252,970 bytes. Embedded checkpoint timestamp 2026-09-12T20:28:14.397488+00:00 (2026-09-13 PKT), Ultralytics 8.4.133, YOLOv8n, exact nine requested class IDs/names. Model weights were read, not modified or committed.
- **Exact supplied val results:** mAP50 **0.7397368457536011**, mAP50-95 **0.46414946906810284**. AP50: hole 0.8712318181290445; weave_error 0.6731388390517081; stain 0.9608333333333333; foreign_object 0.7993684210526317; crease 0.665; edge_damage 0.495; pattern_break 0.7135855087084896. IDs 7/8 have no data and N/A metrics, not zero measured accuracy. JSON takes precedence over rounded chat values; it was not changed. These are supplied validation results, not locally rerun full mAP.
- **Provenance cautions:** checkpoint run name still says `baseline_v1_20260912T195035Z_b9c437aa`; v2 designation follows the user's explicit handoff. Embedded metrics 0.73909/0.4638 differ slightly from the separate JSON; no explanation is invented. Intended dataset is the 2026-09-12 edge-revision-2 manifest `160705b5c2ba87051dbb2600f8db2d8f2863326575ed3847814195352d29705a`, but the checkpoint/report do not embed that hash. `run_metadata.json` remains needed to verify actual training ZIP lineage. Stripped epoch -1 is not a completed-epoch count. AMP remains enabled in saved configuration; finite validation metrics/inference do not prove every training loss was finite. Prototype adoption is not a claim that the historical loss cause was fixed.
- **Shared detector:** repository-relative best.pt default, no pretrained/network fallback, trusted local checkpoint requirement, exact taxonomy validation, default CPU/confidence 0.25/IoU 0.7, 640-square letterboxing and boxes in original input coordinates. Draw class/confidence overlays with OpenCV on frame copies; emit checkpoint hash and settings. `.env.example` now points its planned YOLO path to the same file; CLIs use explicit flags/defaults, not automatic .env loading.
- **File video:** load one detector, process every frame with bounded memory, preserve even frame dimensions/FPS, MP4/mp4v or AVI/MJPG, refuse overwrite/invalid timing/known truncation. Fully decode encoded frames before publishing; retain per-frame JSONL and summary alongside staged diagnostic artifacts. Audio, live cameras/reconnect, ROI/tiling, tracking, database/WebSocket integration and full fused inference contracts remain out of scope.
- **Real verification:** fixed first two strong real val examples per populated class, 14 images at confidence 0.25, produced 13 boxes. No raw source videos found; created an explicitly labeled 28-frame real-validation-image slideshow, then annotated it with the actual checkpoint: 26 boxes on 18 frames; all 28 output frames decode at 640x640/4 FPS. Not factory video or a speed benchmark. Inspected comparison sheet, native annotated images and decoded video frame: aligned hole/stain/foreign-object and some crease/pattern detections, with readable names/confidences. Also recorded misses/class confusion: both selected weave and edge samples missed at 0.25, one hole predicted as weave_error, one stain as foreign_object, overlapping crease predictions. Correct output geometry does not imply perfect predictions; no thresholds were tuned and no final-test samples were touched.
- **Known weakness:** edge AP50 0.495 and recall 0.5, based on only two val tiles from one source strip; both miss at the prototype inference threshold. Small sample size limits confidence, not proof of a sole causal explanation. Crease scarcity, correlated tiles, synthetic/domain bias and weak annotations remain. See model README for exact metric table, provenance, reproducible commands and limitations.
- Five new deterministic inference/IO tests pass, including missing checkpoint, taxonomy rejection, malformed/nonfinite inputs/predictions, overlay copy behavior, video timing/frame accounting and no-overwrite. Actual checkpoint verification is separate from mocked fixtures. AI pip check passes; no dependencies changed. Generated evidence is in ignored `artifacts/inference_v2/` (comparison.jpg, annotated.mp4, verification.json and decoded frames). No retraining, commit, push or branch/ruleset change.
- Final Ruff and Black checks pass for `ai/inference` and `edge` (seven Python files); `git diff --check` passes with only Windows line-ending notices. A direct image CLI run from outside the repo root loads the same checkpoint and detects the real TILDA stain at confidence 0.9123408198356628 (`artifacts/inference_v2/cli_real.png`); the direct video CLI import/argument path also works. An earlier CLI-only check used an already annotated image and is excluded from all real-sample counts/evaluation. Weights and supplied metrics JSON retain their hashes; metrics JSON SHA-256 is `a17919be8e186dc316ca4ff9f26fd1e8186a14c033bdb372c5f0c64a1946ed9e`. Existing backend/frontend CI, full preprocessing verification and GPU training were not rerun because their code/data were unchanged. Model card, Memory, Phases, Architecture, PRD, dataset/root READMEs and the example YOLO path reflect the prototype state; Rules and website Design remain applicable unchanged.
